...
pub fn generate_normal(game:Game, scale: f32, py: Python) -> String {
    let variable = if let Some(_) = game.variable { true } else { false };
...
"#);
    ...
    let p2_moves = player2_string(game, py);
    //let game_matrix = create_matrix(game, variable, py);



    let end = format!("\n\\end{{game}}\n\\end{{{scale}}}");

    begin + &*config + &*p2_moves + &*end
}
...
fn create_matrix(game: Game, variable: bool, py: Python) -> String {
    let mut matrix_str = String::new();
    let actions = game.player[1].borrow(py).actions.clone();
    // loop through
    matrix_str.push_str(&*actions[0].clone());
